...
- **OpenAI gpt-5.2** (all nutrition features): Uses Replit AI Integrations (AI_INTEGRATIONS_OPENAI_API_KEY, AI_INTEGRATIONS_OPENAI_BASE_URL)
  ...
  - Photo analysis: Call A (gpt-5.2 vision identifies items with 200+ approved categories) + server-side deterministic macro calculator (macroCalculator.ts) using USDA table
...
- February 2026: Bodybuilding-Focused Photo Analysis Upgrade
  ...
  - Server-side macro calculator (macroCalculator.ts) replaces LLM Call B with deterministic USDA table math
  ...
  - Calories calculated as P*4 + C*4 + F*9 (always consistent)
  - Call A: gpt-5.2 vision, max_completion_tokens: 500; macros: server-side calculator (no LLM)
